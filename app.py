...
def draw_text_on_image(name, team, cert_id, name_y, team_y, id_y, font_path, font_size, text_color, template):
    ...
    small_font = ImageFont.truetype(font_path, int(font_size / 1.8))
...
if user_type == "Participant":
    try:
        participants_df = pd.read_csv("h4bsub_cleaned.csv")
...
font_paths = {
    "Montserrat": {
        "Black": "./Montserrat/static/agrandir-wide-bold.ttf",
        "SemiBold": "./Montserrat/static/agrandir-wide-bold.ttf",
        "Bold": "./Montserrat/static/agrandir-wide-bold.ttf"
    },
    "Roboto": {
